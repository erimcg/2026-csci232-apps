...
const links = [
    { label: 'Home', path: '/home', key: '/home', position: 'left' },
    { label: 'Chat', path: '/chat', key: '/chat', position: 'left' }
]

const leftLinks = computed(() => {
    if (user.value.username) {
        return links.filter(entry => entry.position == 'left')
    }
...
const rightLinks = computed(() => {
    ...
    if (user.value.username) {
        ...
        links.push({ label: 'Create Account', path: '/create-account', key: '/create-account'})
    }
    else if (route.path == '/create-account') {
        links.push({ label: 'Sign in', path: '/login', key: '/login'})
    }
